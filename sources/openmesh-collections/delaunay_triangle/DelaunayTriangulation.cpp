...
void DelaunayTriangulation::splitTriangle(Triangle* triangle, Vector3D* dot)
{
    ...
    localOptimization(triangle, triangle->Neighbor[1]);
    localOptimization(newTriangle1, newTriangle1->Neighbor[1]);
    localOptimization(newTriangle2, newTriangle2->Neighbor[1]);
}
...
void DelaunayTriangulation::localOptimization(Triangle* t0, Triangle* t1)
{
    _Statistics[1]++;
...
bool DelaunayTriangulation::SwapDiagonal(Triangle* t0, Triangle* t1)
{
    for (int j = 0; j < 3; j++)
    {
        for (int k = 0; k < 3; k++)
        {
                ...
                t1->Vertex[k] != t0->Vertex[2])
            {
                ...
                localOptimization(t0, t0->Neighbor[j]);
                localOptimization(t0, t0->Neighbor[(j + 1) % 3]);
                localOptimization(t1, t1->Neighbor[k]);
                localOptimization(t1, t1->Neighbor[(k + 1) % 3]);
